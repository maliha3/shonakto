...
from .serializers import (
    ...
    FoundPersonUploadSerializer,
    LostPersonSerializer,
    LostPersonUploadSerializer,
)
...
class IsAllowedLostReporter(permissions.BasePermission):
    """Same rule as IsAllowedUploader, but object ownership is via reported_by."""
...
class LostPersonViewSet(viewsets.ModelViewSet):
    """
    /api/lost-persons/          GET (list active records), POST (upload)
    /api/lost-persons/{id}/     GET, PATCH, DELETE (own records only)
    """

    queryset = LostPerson.objects.filter(is_active=True).select_related("reported_by")
    permission_classes = [IsAllowedLostReporter]
    parser_classes = [MultiPartParser, FormParser]

    def get_serializer_class(self):
        if self.action == "create":
            return LostPersonUploadSerializer
        return LostPersonSerializer
